...
# @Last Modified time: 2018-03-09 22:15:30
...
def main(args):
    ...
    res_acc_org = np.zeros((rep,len(qs),len(etas))) # Accuracy of clustering
    res_mean_acc_org = np.zeros((rep,len(qs),len(etas))) # Mean accuracy of clustering (per cluster)
    # res_err = np.zeros((rep,len(qs),len(etas))) # Number of misclustered points
    res_fail_org = np.zeros((rep,len(qs),len(etas))) # Number of Failure
    ...
    for i_rep in xrange(rep):
        ...
        for i_q,q in enumerate(qs):
            ...
            for i_eta,eta in enumerate(etas):
                ...
                algo_org.set_params(eta,beta,q=q)
                ...
                res_acc_org[i_rep,i_q,i_eta] = accuracy(y_true,y_pred_perm_org)
                res_mean_acc_org[i_rep,i_q,i_eta] = mean_accuracy(y_true,y_pred_perm_org)
                ...
                if (i_rep == i_plot) and (m<=2) and (not plotted):
                    ...
                    # title_org = r"SSAC(original) with {} weak oracle ($q={},\eta={}, \beta={}$)".format(weak,q,eta,beta)
                    # f_name_org = res_dir+'/fig_org_n{}_m{}_k{}_q{:03d}_e{:d}.png'.format(n,m,k,int(100*q),int(eta))
    ...
    print_eval("Accuracy(%)",res_acc,etas,
               res_dir+'/res_{}_n{}_m{}_k{}.csv'.format("acc",n,m,k),weak=weak,params=qs)
    print_eval("Mean Accuracy(%)",res_mean_acc,etas,
               res_dir+'/res_{}_n{}_m{}_k{}.csv'.format("meanacc",n,m,k),weak=weak,params=qs)
    ...
    print_eval("Accuracy(%)",res_acc_org,etas,
               res_dir+'/res_org_{}_n{}_m{}_k{}.csv'.format("acc",n,m,k),weak=weak,params=qs)
    print_eval("Mean Accuracy(%)",res_mean_acc_org,etas,
               res_dir+'/res_org_{}_n{}_m{}_k{}.csv'.format("meanacc",n,m,k),weak=weak,params=qs)
    # print_eval("# Error(%)",res_err,qs,etas,
    #            res_dir+'/res_{}_n{}_m{}_k{}.csv'.format("err",n,m,k))
    ...
    plot_eval("Accuracy(%)",res_acc,etas,fig_name,weak=weak,params=qs,res_org=res_acc_org)

    # Plot Mean Accuracy vs. eta
    fig_name = res_dir+'/fig_{}_n{}_m{}_k{}.pdf'.format("meanacc",n,m,k)
    plot_eval("Mean Accuracy(%)",res_mean_acc,etas,fig_name,weak=weak,params=qs,res_org=res_mean_acc_org)

    # Plot Failure vs. eta
    fig_name = res_dir+'/fig_{}_n{}_m{}_k{}.pdf'.format("fail",n,m,k)
    plot_eval("# Failures",res_fail,etas,fig_name,is_sum=True,weak=weak,params=qs,res_org=res_fail_org)
