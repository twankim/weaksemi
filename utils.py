...
# @Last Modified time: 2017-10-21 02:06:10
...
def plot_cluster(X,y_true,y_pred,k,mpps,gamma,title,f_name,verbose):
    ...
    for i in xrange(1,k+1):
        idx = y_true==i
        plt.scatter(X[idx,0],X[idx,1],c=cmap(i),label=classes[i],edgecolor='black',linewidth=1)
    ...
    for t in xrange(k):
        mpp = mpps[t]
        plt.plot(mpp[0],mpp[1],'w^',ms=15,edgecolor='black',linewidth=1)
